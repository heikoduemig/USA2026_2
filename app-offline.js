...
function imageSearch(p){ return `https://www.google.com/search?tbm=isch&q=${encodeURIComponent((p.query || p.name + ' ' + p.city) + ' exterior official photos')}`; }
...
function card(p, idx) {
  return `<article class="place ${categoryClass(p.category)}" id="${slug(p.name)}">
    <button class="photo" onclick="selectPlace(${idx})" style="background-image:linear-gradient(180deg,rgba(0,0,0,.02),rgba(0,0,0,.72)),url('${p.image}')">
      <span class="tier">${p.priority}</span>
      <span class="image-label">${p.imageLabel || 'Location-Bild'}</span>
    </button>
    <div class="body">
      <h3>${p.name}</h3>
...
function marker(p, idx){
  ...
  return `<button class="place-pin ${categoryClass(p.category)}${active}" style="left:${p.map.x}%;top:${p.map.y}%" onclick="selectPlace(${idx})" title="${p.name}" aria-label="${p.name}">
    <span class="pin-core"></span>
    <span class="place-label">${p.name}<small>${p.city}</small></span>
  </button>`;
}
...
function popup(){
  ...
  return `<aside class="map-popup">
    <button class="popup-close" onclick="clearPlace()" aria-label="Schließen">×</button>
    <div class="popup-photo" style="background-image:linear-gradient(180deg,transparent,rgba(0,0,0,.66)),url('${p.image}')"></div>
    <div class="popup-body">
      <div class="eyebrow">${p.city} · ${p.category}</div>
      <h3>${p.name}</h3>
...
function renderMap(){
  ...
  el.innerHTML = `<svg class="route-svg" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
    <defs>
      <linearGradient id="routeGrad" x1="0" x2="1"><stop offset="0" stop-color="#ff4fd8"/><stop offset="1" stop-color="#46d6e6"/></linearGradient>
    </defs>
  ...
  <div class="legend">
    <span><i class="cat-table"></i> Tabledance</span>
    <span><i class="cat-gentlemen"></i> Gentlemen</span>
    <span><i class="cat-show"></i> Show</span>
    <span><i class="cat-casino"></i> Casino</span>
  </div>
  ${popup()}`;
  const mode = onlineState ? 'Online-Modus aktiv' : 'Offline-Modus aktiv';
  status(`${mode}: ${shown.length} Location-Marker auf der Karte.`);
}
...
function focusCity(city){
  ...
  activePlace = null;
  renderMap();
  document.getElementById(slug(city))?.scrollIntoView({behavior:'smooth', block:'start'});
}
...
function nearMe(){
  alert(onlineState ? 'Online-Modus: Bilder, Websites und Google-Maps-Links werden live geladen.' : 'Offline-Modus: gespeicherte Inhalte bleiben sichtbar. Externe Links funktionieren wieder bei Empfang.');
}
